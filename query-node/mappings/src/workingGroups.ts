...
async function getWorkingGroup(
  ...
  return getWorkingGroupByName(
    store,
    groupName as WorkingGroupModuleName,
    relations
  );
}
...
async function getOpening(
  ...
  const opening = await store.get(WorkingGroupOpening, {
    where: { id: openingstoreId },
    relations,
  });
  if (!opening) {
...
async function getApplication(
  ...
  const application = await store.get(WorkingGroupApplication, {
    where: { id: applicationstoreId },
  });
  if (!application) {
...
async function getApplicationFormQuestions(
  ...
  if (!openingWithQuestions.metadata.applicationFormQuestions) {
    return inconsistentState(
      "Application form questions not found for opening",
      openingstoreId
    );
  }
...
const InputTypeToApplicationFormQuestionType = {
  [OpeningMetadata.ApplicationFormQuestion.InputType.TEXT]:
    ApplicationFormQuestionType.TEXT,
  [OpeningMetadata.ApplicationFormQuestion.InputType.TEXTAREA]:
    ApplicationFormQuestionType.TEXTAREA,
};

function parseQuestionInputType(
  type?: OpeningMetadata.ApplicationFormQuestion.InputType | null
): ApplicationFormQuestionType {
  const validType: OpeningMetadata.ApplicationFormQuestion.InputType =
    type || 0;
...
export async function createWorkingGroupOpeningMetadata(
  ...
  if (originalMeta instanceof Bytes) {
    const deserializedMetadata = await deserializeMetadata(
      OpeningMetadata,
      originalMeta
    );
  ...
  const openingMetadata = new WorkingGroupOpeningMetadata({
    ...
    expectedEnding: expectedEndingTimestamp
      ? new Date(expectedEndingTimestamp * 1000)
      : undefined,
    applicationFormQuestions: [],
  });
...
async function createApplicationQuestionAnswers(
  ...
  const questions = await getApplicationFormQuestions(
    store,
    application.opening.id
  );
  ...
  await Promise.all(
    (answers || []).slice(0, questions.length).map(async (answer, index) => {
      ...
      await store.save<ApplicationFormQuestionAnswer>(
        applicationFormQuestionAnswer
      );
...
async function handleAddUpcomingOpeningAction(
  ...
  const upcomingOpening = new UpcomingWorkingGroupOpening({
    ...
    group,
    rewardPerBlock:
      isSet(rewardPerBlock) && parseInt(rewardPerBlock)
        ? new BN(rewardPerBlock)
        : undefined,
    expectedStart: expectedStart ? new Date(expectedStart * 1000) : undefined,
    stakeAmount:
      isSet(minApplicationStake) && parseInt(minApplicationStake)
        ? new BN(minApplicationStake)
        : undefined,
...
async function handleSetWorkingGroupMetadataAction(
  ...
  const setNewOptionalString = (field: keyof IWorkingGroupMetadata) =>
    typeof newMetadata?.[field] === "string"
...
async function handleWorkingGroupMetadataAction(
  ...
  if (action.addUpcomingOpening) {
    return handleAddUpcomingOpeningAction(
      store,
      event,
      statusChangedEvent,
    ...
    return handleRemoveUpcomingOpeningAction(
      store,
      action.removeUpcomingOpening
    );
  } else if (action.setGroupMetadata) {
    return handleSetWorkingGroupMetadataAction(
      store,
      event,
      statusChangedEvent,
...
async function handleTerminatedWorker({
  store,
  event,
  ...
  const [
    ...
    optPenalty,
    optRationale,
  ] = new WorkingGroup_TerminatedWorkerEvent_V1001(event).params;
  const group = await getWorkingGroup(store, event, ["leader"]);
  const worker = await getWorker(
    store,
    group.name as WorkingGroupModuleName,
    workerId,
    ["application", "workinggroupleader"]
  );
  ...
  if (worker.isLead) {
    const terminatedLeaderEvent = new TerminatedLeaderEvent({
      ...
      rationale: optRationale.isSome
        ? bytesToString(optRationale.unwrap())
        : undefined,
    });
...
export async function findLeaderSetEventByTxHash(
  store: DatabaseManager,
  txHash?: string
): Promise<LeaderSetEvent> {
  const leaderSetEvent = await store.get(LeaderSetEvent, {
    where: { inExtrinsic: txHash },
  });
...
// Mapping functions
export async function workingGroups_OpeningAdded({
  store,
  event,
}: EventContext & StoreContext): Promise<void> {
  const [
  ...
  const opening = new WorkingGroupOpening({
    ...
    type: openingType.isLeader
      ? WorkingGroupOpeningType.LEADER
      : WorkingGroupOpeningType.REGULAR,
  });

  const metadata = await createWorkingGroupOpeningMetadata(
    store,
    eventTime,
    metadataBytes
  );
...
export async function workingGroups_AppliedOnOpening({
  store,
  event,
}: EventContext & StoreContext): Promise<void> {
  const [
...
export async function workingGroups_LeaderSet({
  store,
  event,
}: EventContext & StoreContext): Promise<void> {
  const group = await getWorkingGroup(store, event);
...
export async function workingGroups_OpeningFilled({
  store,
  event,
}: EventContext & StoreContext): Promise<void> {
  const [
    ...
    applicationIdsSet,
  ] = new WorkingGroup_OpeningFilledEvent_V1001(event).params;

  const group = await getWorkingGroup(store, event);
  const opening = await getOpening(
    store,
    `${group.name}-${openingRuntimeId.toString()}`,
    ["applications", "applications.applicant"]
  );
  ...
  if (
    opening.type === WorkingGroupOpeningType.LEADER &&
  ...
  const hiredWorkers = (
    await Promise.all(
      (opening.applications || [])
        ...
        .filter(
          (application) =>
            application.status.isTypeOf !== "ApplicationStatusWithdrawn"
        )
        .map(async (application) => {
          const isAccepted = acceptedApplicationIds.some(
            (runtimeId) => runtimeId.toNumber() === application.runtimeId
          );
          const applicationStatus = isAccepted
            ? new ApplicationStatusAccepted()
            : new ApplicationStatusRejected();
          ...
          if (isAccepted) {
            ...
            const [, workerRuntimeId] =
              Array.from(applicationIdToWorkerIdMap.entries()).find(
                ([applicationRuntimeId]) =>
                  applicationRuntimeId.toNumber() === application.runtimeId
              ) || [];
            if (!workerRuntimeId) {
              return inconsistentState(
  ...
  if (opening.type === WorkingGroupOpeningType.LEADER && hiredWorkers.length) {
    ...
    const leaderSetEvent = await findLeaderSetEventByTxHash(
      store,
      openingFilledEvent.inExtrinsic
    );
...
export async function workingGroups_OpeningCanceled({
  store,
  event,
}: EventContext & StoreContext): Promise<void> {
  const [openingRuntimeId] = new WorkingGroup_OpeningCanceledEvent_V1001(
    event
  ).params;
...
export async function workingGroups_ApplicationWithdrawn({
  store,
  event,
}: EventContext & StoreContext): Promise<void> {
  const [
    applicationRuntimeId,
  ] = new WorkingGroup_ApplicationWithdrawnEvent_V1001(event).params;

  const group = await getWorkingGroup(store, event);
  const application = await getApplication(
    store,
    `${group.name}-${applicationRuntimeId.toString()}`
  );
...
export async function workingGroups_StatusTextChanged({
  store,
  event,
}: EventContext & StoreContext): Promise<void> {
  const [, optBytes] = new WorkingGroup_StatusTextChangedEvent_V1001(
    event
  ).params;
  const group = await getWorkingGroup(store, event);
  ...
  if (optBytes.isSome) {
    ...
    if (metadata) {
      result = await handleWorkingGroupMetadataAction(
        store,
        event,
        statusTextChangedEvent,
        metadata
...
export async function workingGroups_LeadRemarked({
  store,
  event,
}: EventContext & StoreContext): Promise<void> {
  ...
  if (metadata?.moderatePost) {
    if (group.name !== "forumWorkingGroup") {
      return invalidMetadata(
        `The ${group.name} is incompatible with the remarked moderatePost`
      );
    }
...
export async function workingGroups_WorkerRemarked({
  store,
  event,
}: EventContext & StoreContext): Promise<void> {
  const [workerId, metadataByte] = new WorkingGroup_WorkerRemarkedEvent_V1001(
    event
  ).params;
  const group = await getWorkingGroup(store, event);
  ...
  if (metadata?.moderatePost) {
    ...
    const {memberId } = metadata.verifyValidator;
...
export async function workingGroups_WorkerRoleAccountUpdated({
  ...
  const [
    workerId,
    accountId,
  ] = new WorkingGroup_WorkerRoleAccountUpdatedEvent_V1001(event).params;
  const group = await getWorkingGroup(store, event);
  const worker = await getWorker(
    store,
    group.name as WorkingGroupModuleName,
  ...
  await store.save<WorkerRoleAccountUpdatedEvent>(
    workerRoleAccountUpdatedEvent
  );
...
export async function workingGroups_WorkerRewardAccountUpdated({
  ...
  const [
    workerId,
    accountId,
  ] = new WorkingGroup_WorkerRewardAccountUpdatedEvent_V1001(event).params;
  const group = await getWorkingGroup(store, event);
  const worker = await getWorker(
    store,
    group.name as WorkingGroupModuleName,
...
export async function workingGroups_StakeIncreased({
  store,
  event,
}: EventContext & StoreContext): Promise<void> {
  const [workerId, increaseAmount] = new WorkingGroup_StakeIncreasedEvent_V1001(
    event
  ).params;
  const group = await getWorkingGroup(store, event);
  const worker = await getWorker(
...
export async function workingGroups_RewardPaid({
  store,
  event,
}: EventContext & StoreContext): Promise<void> {
  const [
    ...
    amount,
    rewardPaymentType,
  ] = new WorkingGroup_RewardPaidEvent_V1001(event).params;
  const group = await getWorkingGroup(store, event);
  const worker = await getWorker(
    store,
    group.name as WorkingGroupModuleName,
  ...
  const rewardPaidEvent = new RewardPaidEvent({
    ...
    amount,
    rewardAccount: rewardAccountId.toString(),
    paymentType: rewardPaymentType.isRegularReward
      ? RewardPaymentType.REGULAR
      : RewardPaymentType.MISSED,
  });
...
export async function workingGroups_NewMissedRewardLevelReached({
  ...
  const [
    workerId,
    newMissedRewardAmountOpt,
  ] = new WorkingGroup_NewMissedRewardLevelReachedEvent_V1001(event).params;
  const group = await getWorkingGroup(store, event);
  const worker = await getWorker(
    store,
    group.name as WorkingGroupModuleName,
  ...
  await store.save<NewMissedRewardLevelReachedEvent>(
    newMissedRewardLevelReachedEvent
  );
...
export async function workingGroups_WorkerExited({
  store,
  event,
}: EventContext & StoreContext): Promise<void> {
...
export async function workingGroups_LeaderUnset({
  store,
  event,
}: EventContext & StoreContext): Promise<void> {
  const group = await getWorkingGroup(store, event, ["leader"]);
...
export async function workingGroups_TerminatedWorker(
  ctx: EventContext & StoreContext
): Promise<void> {
  await handleTerminatedWorker(ctx);
}
export async function workingGroups_TerminatedLeader(
  ctx: EventContext & StoreContext
): Promise<void> {
  await handleTerminatedWorker(ctx);
}
...
export async function workingGroups_WorkerRewardAmountUpdated({
  ...
  const [
    workerId,
    newRewardPerBlockOpt,
  ] = new WorkingGroup_WorkerRewardAmountUpdatedEvent_V1001(event).params;
  const group = await getWorkingGroup(store, event);
  const worker = await getWorker(
    store,
    group.name as WorkingGroupModuleName,
  ...
  await store.save<WorkerRewardAmountUpdatedEvent>(
    workerRewardAmountUpdatedEvent
  );
...
export async function workingGroups_StakeSlashed({
  store,
  event,
}: EventContext & StoreContext): Promise<void> {
  const [
    ...
    requestedAmount,
    optRationale,
  ] = new WorkingGroup_StakeSlashedEvent_V1001(event).params;
  const group = await getWorkingGroup(store, event);
  const worker = await getWorker(
    store,
    group.name as WorkingGroupModuleName,
...
export async function workingGroups_StakeDecreased({
  store,
  event,
}: EventContext & StoreContext): Promise<void> {
  const [workerId, amount] = new WorkingGroup_StakeDecreasedEvent_V1001(
    event
  ).params;
  const group = await getWorkingGroup(store, event);
  const worker = await getWorker(
...
export async function workingGroups_WorkerStartedLeaving({
  store,
  event,
}: EventContext & StoreContext): Promise<void> {
  const [
    workerId,
    optRationale,
  ] = new WorkingGroup_WorkerStartedLeavingEvent_V1001(event).params;
  const group = await getWorkingGroup(store, event);
  const worker = await getWorker(
    store,
    group.name as WorkingGroupModuleName,
...
export async function workingGroups_BudgetSet({
  store,
  event,
}: EventContext & StoreContext): Promise<void> {
  const [newBudget] = new StorageWorkingGroup_BudgetSetEvent_V1001(
    event
  ).params;
  const group = await getWorkingGroup(store, event);
...
export async function workingGroups_BudgetSpending({
  store,
  event,
}: EventContext & StoreContext): Promise<void> {
  const [
    ...
    amount,
    optRationale,
  ] = new WorkingGroup_BudgetSpendingEvent_V1001(event).params;
  const group = await getWorkingGroup(store, event);
...
export async function workingGroups_WorkingGroupBudgetFunded({
  ...
  const [
    ...
    amount,
    rationale,
  ] = new WorkingGroup_WorkingGroupBudgetFundedEvent_V1001(event).params;
  const group = await getWorkingGroup(store, event);
