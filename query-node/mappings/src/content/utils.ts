...
export async function unsetAssetRelations(store: DatabaseManager, dataObject: StorageDataObject): Promise<void> {
  ...
  async function unconnectAsset<T extends EntityType<Channel | Video | VideoSubtitle | Playlist>>(
    entity: T,
    targetAsset: string,
    relations: typeof channelAssets | typeof videoAssets | typeof subtitleAssets | typeof playlistAssets
  ) {
    // load video/channel
    ...
    } else if (result instanceof VideoSubtitle) {
      logger.info('Asset has been disconnected from Video Subtitle', {
        subtitleId: result.id.toString(),
        dataObjectId: dataObject.id,
      })
    } else {
      // asset belongs to Playlist
      logger.info('Asset has been disconnected from Playlist', {
  ...
  for (const playlistAsset of playlistAssets) {
    await unconnectAsset(Playlist, playlistAsset, playlistAssets)
  }
