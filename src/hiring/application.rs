...
impl<BlockNumber: Clone> ActiveOpeningStage<BlockNumber> {
	...
	pub fn ensure_active_opening_is_accepting_applications<Err>(&self, error: Err) -> Result<BlockNumber, Err> {
		if let ActiveOpeningStage::AcceptingApplications { started_accepting_applicants_at_block } = self {
	...
	pub fn ensure_active_opening_is_in_review_period <Err>(&self, error: Err) -> Result<(BlockNumber,BlockNumber), Err> {
		match self {
