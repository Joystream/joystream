...
#[test]
fn unsuccessful_update_upcoming_token_sale_non_existing_channel() {
...
#[test]
fn unsuccessful_update_creator_token_sale_member_channel_unauthorized_actors() {
    with_default_mock_builder(|| {
        run_to_block(1);
...
#[test]
fn unsuccessful_update_creator_token_sale_curator_channel_unauthorized_actors() {
    with_default_mock_builder(|| {
        run_to_block(1);
        curators::add_curator_to_new_group(DEFAULT_CURATOR_ID);
...
#[test]
fn successful_update_creator_token_sale_member_channel() {
    with_default_mock_builder(|| {
        run_to_block(1);
        CreateChannelFixture::default().call_and_assert(Ok(()));
        IssueCreatorTokenFixture::default().call_and_assert(Ok(()));
...
#[test]
fn successful_update_creator_token_sale_curator_channel() {
    with_default_mock_builder(|| {
        run_to_block(1);
        curators::add_curator_to_new_group(DEFAULT_CURATOR_ID);
        CreateChannelFixture::default()
