...
export function calculateValidatorsRewardsPerEra(
  ...
  if (stakingRate > IDEAL_STAKING_RATE) {
    validatorsRewardsPerYear =
      totalIssuance *
      (MIN_INFLATION_RATE +
        (MAX_INFLATION_RATE - MIN_INFLATION_RATE) * 2 ** ((IDEAL_STAKING_RATE - stakingRate) / FALL_OFF_RATE))
  } else if (stakingRate === IDEAL_STAKING_RATE) {
    validatorsRewardsPerYear = totalIssuance * MAX_INFLATION_RATE
  } else {
    validatorsRewardsPerYear =
      totalIssuance *
      (MIN_INFLATION_RATE + (MAX_INFLATION_RATE - MIN_INFLATION_RATE) * (stakingRate / IDEAL_STAKING_RATE))
  }
