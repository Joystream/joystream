...
export const formatReward = (
  { amount_per_payout: amount, payout_interval: interval, next_payment_at_block }: RewardRelationship | RewardPolicy,
  showNextPaymentBlock = false
) => {
  const nextPaymentBlock =
    next_payment_at_block instanceof Option ? next_payment_at_block.unwrapOr(null) : next_payment_at_block
