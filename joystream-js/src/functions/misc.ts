...
export const notDefined = (x: any): boolean => x === null || typeof x === 'undefined'

export const isDefined = (x: any): boolean => !notDefined(x)

export const isDef = isDefined

export const notDef = notDefined

export const isObj = (x: any): boolean => x !== null && typeof x === 'object'

export const isStr = (x: any): boolean => typeof x === 'string'

export const isNum = (x: any): boolean => typeof x === 'number'

export const isEmptyStr = (x: any): boolean => notDefined(x) || (isStr(x) && (x as string).trim().length === 0)

export const nonEmptyStr = (x?: any) => isStr(x) && (x as string).trim().length > 0
...
export const nonEmptyArr = (x: any): boolean => Array.isArray(x) && x.length > 0
...
export function findFirstParamOfSubstrateEvent<T extends Codec>(
...
export function isObjectWithProperties<X extends unknown, Y extends PropertyKey[]>(
  input: X,
  ...props: Y
): input is X & Record<Y[number], unknown> {
