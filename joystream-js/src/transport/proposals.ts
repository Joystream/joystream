...
export default class ProposalsTransport extends BaseTransport {
  ...
  async activeProposalsIds() {
    const result = await this.entriesByIds<ProposalId, Null>(this.api.query.proposalsEngine.activeProposalIds)
  ...
  async proposalsBatch(statusFilter: ProposalStatusFilter, batchNumber = 1, batchSize = 5): Promise<ProposalsBatch> {
    const ids = (status === 'Active' ? await this.activeProposalsIds() : await this.proposalsIds()).sort((id1, id2) =>
      id2.cmp(id1)
    ) // Sort by newest
    let rawProposalsWithIds = (await Promise.all(ids.map((id) => this.rawProposalById(id)))).map((proposal, index) => ({
      id: ids[index],
      proposal,
    }))
  ...
  historicalProposalsBatch(
    ...
    return new Promise((resolve, reject) => {
      ...
      const parsedProposals: ParsedProposal[] = proposalsInBatchData.map((proposalData) =>
        this.parseHistoricalProposal(proposalData)
      )
