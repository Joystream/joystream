...
type LoadObjectsByIdsFn<Id extends IdLike, Obj extends HasId> = (ids: Id[]) => Promise<Obj[]>

function anyIdToString(id: IdLike): string {
  return typeof id === 'number' ? new BN(id).toString() : id.toString()
}
...
export class SimpleCache<Id extends IdLike, Obj extends HasId> {
  ...
  clearExcept(keepIds: Id[] | Set<string>): void {
    const prevCacheSize = this.cache.size
    const keepIdsSet = keepIds instanceof Set ? keepIds : new Set(keepIds.map((id) => id.toString()))
