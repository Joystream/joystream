...
contract ContentDirectory is RuntimeManageable, Pausable {
  ...
  enum ContentDirectoryOperation {
  ...
  // Access/validation utils:
    ...
    if (_ownership.isMember()) {
      require(
        membershipBridge.memberExists(_ownership.asMember()),
        "Member ownership - member does not exist!"
      );
    }
  ...
  function updateCuratorGroupPermissions (
    uint16 _groupId,
    bool[] memory _permissions
  ) public whenNotPaused onlyLead {
    require(_permissions.length == GROUP_PERMISSIONS_FLAGS_LENGTH, "Invalid permissions array length");
	...
	function addCuratorToGroup (
		...
		require(curatorGroupStorage.isCuratorInExistingGroup(_curatorId, _groupId) == false, "Curator is already in the group");
	...
	function removeCuratorFromGroup (
		...
		require(curatorGroupStorage.groupExists(_groupId), "Group id not recognized");
		require(curatorGroupStorage.isCuratorInExistingGroup(_curatorId, _groupId) == true, "Curator is already not in the group");
  ...
  // removeVideo - group channel video removal overload
	function removeGroupChannelVideoAsCurator (
		uint64 _videoId,
    uint64 _curatorId
	) public whenNotPaused {
		Video memory video = videoStorage.getExistingVideo(_videoId);
