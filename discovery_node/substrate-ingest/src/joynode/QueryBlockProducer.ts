...
export default class QueryBlockProducer extends EventEmitter {
    ...
    private async _produce_blocks() {
        if (this._producing_blocks_blocks) {
            logger.warn("Already producing new blocks");
            return;
        }
        ...
        while (this._block_to_be_produced_next <= this._height_of_chain) {
            logger.debug(`Fetching block #${this._block_to_be_produced_next}`);

            try {
                let hash = this._query_service.getBlockHash(this._block_to_be_produced_next);
                await hash.then((hash) => console.log(hash));
                //logger.debug(`\tHash ${block_hash_of_target.toString()}.`);
            } catch (e) {
                logger.error(e);
                continue;
            }
            ...
            logger.debug(`\tRead ${records.length} events.`);
            ...
            logger.debug(`\tFetched full block.`);
