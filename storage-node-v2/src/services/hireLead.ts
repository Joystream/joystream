...
export async function hireStorageWorkingGroupLead(): Promise<void> {
  ...
  await sendAndFollowTx(
    ...
    SudoKeyPair,
    sudo(
      api.tx.storageWorkingGroup.addOpening(
  ...
  await sendAndFollowTx(
    api,
    LeadKeyPair,
    api.tx.storageWorkingGroup.applyOnOpening(
      memberId, // member id
  ...
  await sendAndFollowTx(
    api,
    SudoKeyPair,
    sudo(api.tx.storageWorkingGroup.beginApplicantReview(newOpeningId))
  )
  ...
  await sendAndFollowTx(
    ...
    SudoKeyPair,
    sudo(
      api.tx.storageWorkingGroup.fillOpening(
        newOpeningId, // opening id
