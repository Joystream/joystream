...
import chanelPlaylists from '../flows/content/channelPlaylists'
...
scenario('Content directory', async ({ job }) => {
  ...
  job('channel playlists', chanelPlaylists).after(videoCountersJob)
  job('nft auction and offers', nftAuctionAndOffers).after(leadSetupJob)
})
