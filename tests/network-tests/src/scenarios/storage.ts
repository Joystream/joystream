...
import { copyAndEmptyBuckets } from '../flows/storage/copyAndEmptyBuckets'

// eslint-disable-next-line @typescript-eslint/no-floating-promises
scenario('Storage', async ({ job }) => {
  const setReplicationRateJob = job('set replication rate', setReplicationRate)
  const copyAndEmptyBucketsJob = job('copy/empty buckets', copyAndEmptyBuckets).after(setReplicationRateJob)
  const storageSyncJob = job('storage sync', storageSync).after(copyAndEmptyBucketsJob)
