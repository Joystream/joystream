...
export class RemarkModeratePostsFixture extends WithForumWorkersFixture {
  ...
  public constructor(
    api: Api,
    query: QueryNodeApi,
    moderations: RemarkPostModerationInput[]
  ) {
    super(api, query);
  ...
  protected assertQueriedPostsAreValid(
    ...
    const moderatedSuccessfully = this.moderations.filter(
      (m) => !m.expectFailure
    ).length;
    assert.equal(
      qEvents.length,
      moderatedSuccessfully,
      "Too many posts were moderated"
    );
    ...
    this.events.map((e, i) => {
      ...
      Utils.assert(
        qPost.status.__typename === "PostStatusModerated",
        "Invalid post status"
      );
      Utils.assert(
        qPost.status.postModeratedEvent,
        "Query node: Missing PostModeratedEvent ref"
      );
  ...
  protected assertQueryNodeEventIsValid(
    qEvent: PostModeratedEventFieldsFragment,
    i: number
  ): void {
    ...
    assert.equal(
      qEvent.actor.id,
      `forumWorkingGroup-${
        asWorker ? asWorker.toString() : this.forumLeadId!.toString()
      }`
    );
  ...
  async runQueryNodeChecks(): Promise<void> {
    ...
    const expectFailureAtIndexes = this.moderations.flatMap((m, i) =>
      m.expectFailure ? [i] : []
    );
    // Query the events
    const qEvents = await this.query.tryQueryWithTimeout(
      ...
      (qEvents) =>
        this.assertQueryNodeEventsAreValid(qEvents, expectFailureAtIndexes)
    );

    // Query the threads
    const qPosts = await this.query.getPostsByIds(
      this.moderations.map((m) => m.postId)
    );
