...
export class UpdateProfileHappyCaseFixture extends BaseQueryNodeFixture {
  ...
  private assertProfileUpdateSuccesful(
    ...
    assert.isFalse(
      Utils.hasDuplicates(metadata.externalResources?.map(({ type }) => type))
    );
  }

  public getExpectedValues(): MemberProfileData {
    return {
      handle: isSet(this.newValues.handle)
        ? this.newValues.handle
        : this.oldValues.handle,
      name: isSet(this.newValues.name)
        ? this.newValues.name || null
        : this.oldValues.name,
      about: isSet(this.newValues.about)
        ? this.newValues.about || null
        : this.oldValues.about,
  ...
  private assertQueryNodeEventIsValid(
    ...
    assert.equal(
      newMetadata.avatar?.avatarUri,
      expected.avatarUri || undefined
    );
    assert.includeDeepMembers(
  ...
  async execute(): Promise<void> {
    ...
    const txFee = await this.api.estimateTxFee(
      this.tx,
      this.memberContext.account
    );
    await this.api.treasuryTransferBalance(this.memberContext.account, txFee);
    const txRes = await this.api.signAndSend(
      this.tx,
      this.memberContext.account
    );
    this.event = await this.api.getEventDetails(
      txRes,
      "members",
