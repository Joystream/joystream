...
{
  constructor(own_keypair, peer_pubkey, challenge_size, message_offset)
  {
    this.RESERVED_MESSAGE_SLOTS = 10; // Number of message slots reserved.

    this.own_keypair = own_keypair;
    this.peer_pubkey = peer_pubkey;
    this.challenge_size = challenge_size || 32;
    this.message_offset = message_offset || 0;
    ...
    this.handlers = {};
    this.handlers[this.message_offset + MSG_CHALLENGE] = this.handle_challenge.bind(this);
    this.handlers[this.message_offset + MSG_RESPONSE]  = this.handle_response.bind(this);
    this.handlers[this.message_offset + MSG_FINALIZE]  = this.handle_finalize.bind(this);
  ...
  initiate(cb)
  {
    ...
    // Message flag
    var offset = 0;
    msg[offset++] = this.message_offset + MSG_CHALLENGE;
  ...
  handle_response(message, cb)
  {
    ...
    if (this.state.last_message[0] != this.message_offset + MSG_CHALLENGE) {
  ...
  handle_finalize(message, cb)
  {
    ...
    if (this.state.last_message[0] != this.message_offset + MSG_RESPONSE) {
  ...
  _challenge_response(message, decrypt_iv, offset, next_message, mutate_step)
  {
    ...
    if (next_message) {
      ...
      // Message flag
      var offset = 0;
      msg[offset++] = this.message_offset + next_message;
