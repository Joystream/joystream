    ...
    try {
      if (mode == 'r') {
        ...
        stream.once('readable', () => {
          debug('Reading', file_type.minimumBytes, 'Bytes from the stream.');
          stream.pause();
          const chunk = stream.read(file_type.minimumBytes);
          var data = chunk;
          ...
          if (null != data) {
            const res = file_type(data);
            if (res != null) {
              ft = res;
            }
          }
          debug('Detected file type is', ft.mime);

          cb(null, ft.mime, stream);
